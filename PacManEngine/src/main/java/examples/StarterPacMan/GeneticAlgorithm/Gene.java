...
public class Gene implements Comparable<Gene>{
	...
	public Gene[] reproduce(Gene other){
		...
		Random rand = new Random();
		int crossover = 1 + rand.nextInt(GeneticAlgorithm.CHROMOSOME_SIZE - 2); // At least one crossover, range is [1, size-1]
		Gene first = new Gene();
		Gene second = new Gene();
		for (int i = 0; i < GeneticAlgorithm.CHROMOSOME_SIZE; i++) {
			if (i < crossover) {
				first.mChromosome[i] = this.mChromosome[i];
				second.mChromosome[i] = other.mChromosome[i];
			} else {
				first.mChromosome[i] = other.mChromosome[i];
				second.mChromosome[i] = this.mChromosome[i];
			}
		}
		result[0] = first;
		result[1] = second;
...
//	}
	...
	public PhenoBean getPhenoType() {
		return new PhenoBean(mChromosome);
	}
