...
public class GeneticAlgorithm {
	...
	public void evaluateGeneration(){
		for(int i = 0; i < mPopulation.size(); i++){
			...
			Executor exec = new Executor.Builder()
            .setTickLimit(4000)
            .setSightLimit(500)
            .build();;
            ...
            Stats[] result = exec.runExperiment(new EvolvedPacMan(g.getPhenoType()), new POCommGhosts(50), EVALUATION_ITERATIONS, "");
            //				System.out.println(j + ": Result = " + result.score + ", level: " + (result.getLevel()) + ", time: " + result.totalTime + ", points per time: " + ppt);  
            avg = result[0].getAverage();
