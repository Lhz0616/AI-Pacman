...
//                    System.out.println("K: " + k + " W: " + currentWeight);
                ...
                totalInputs += neuron.weights.get(numInputs - 1);
                outputs.add(calculateRelu(totalInputs));
                currentWeight = 0;
            }
        }
        return calculateSoftmax(outputs);
    }

    private ArrayList<Double> calculateSoftmax(ArrayList<Double> outputs){
        double max = outputs
            .stream()
        ...
        double sumExp = outputs
            .stream()
            .mapToDouble(value -> Math.exp(value - max))
            .sum();

        outputs.replaceAll(value -> Math.exp(value - max) / sumExp);

        return outputs;
    }

    private double calculateRelu(double activation){
    ...
    public static NeuralNet readFromFile(String filename) throws IOException {
        ...
        scanner.close();
        System.out.println("Line 1:" + lines.get(0));
        System.out.println("Line 2:" + lines.get(1).split(",").length);
